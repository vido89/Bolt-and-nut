...
customHead = "hex"; // [button, hex, flatSocket, flatHead, conical, socket, set, grub]
customThread = "metric"; // [none, metric]
customTolerance = -0.5; // [-0.9:0.05:0.9]
customPitch = 0.5; // Set pitch to 0.5 mm
...
//bolt(v=true);
// Create the bolt with the specified parameters
bolt(size = metric_fastener[customSize], 
     length = customLength, 
     head = customHead, 
     threadType = customThread, 
     tolerance = customTolerance, 
     pitch = customPitch, // Add pitch parameter
     list = true);

// Try these:
//boltHole(v = true, center = true, 2d = true, tolerance = .4);
translate([15, 0, 0])
nut(size = metric_fastener[customSize], v = true, center = true);
